...
class agent {
   public: // <--- FIX 1: Moved public here so we can read the agent's state!
    ...
    point r = {0, 0}; // <--- The actual position variable
...
class simulationinstance {
    ...
    void update() {
        ...
        for (int i = 0; i < playercount; i++) {
            if (!humanmode)
                // FIX 2: Trigger calculate_2 instead of calculate_1
                players[i].calculate_2(raycastagent(players[i]),
   ...
   public:
    ...
    void run() {
        if (visualmode) {
            InitWindow(screenWidth, screenHeight, "AGV Simulator");
            SetTargetFPS(60);
